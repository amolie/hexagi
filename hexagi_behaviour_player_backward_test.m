function hexagi_behaviour_player_backward_test(Subjects,TimePoints,ProcPath,StatsPath,FigPath)

% Gets player location and yaw (orientation) for the trials in the test phase, from fixation end to drop, and checks if they are equally long 
...
if  nargin<1
    ...
    %TimePoints  = 'StandStill';                  % Evaluate only stand still time points - aka, StandStill_nonBackward
...
for iSub    = 1:nSubs
    ...
    for iRun = 1:2
        ...
        for iTrial = 1:length(Test.CueStart)
            
            TimeOfInterest  = PlayerTest.LocTime >= Test.FixEnd(iTrial) & PlayerTest.LocTime <= Test.Drop(iTrial);
            Trial           = PlayerTest(TimeOfInterest,:);
...
for iSub  = 1:nSubs
    ...
    if  ~exist(fullfile(ProcPath,sprintf('Sub%d',SubID),'\Test\Player\Backward'),'dir')
    mkdir(fullfile(ProcPath,sprintf('Sub%d',SubID),'\Test\Player\Backward')); end
    ...
    for iRun = 1:2
        ...
        switch  TimePoints
            case 'Movement'
                ...
                load(fullfile(ProcPath, sprintf('Sub%d', SubID), 'Test\Player', sprintf('PlayerTestMove%d.mat', iRun)),'PlayerTestMove'); 
                Player = PlayerTestMove;
            ...
            case 'StandStill'
                % Evaluate all time points including stand still
                load(fullfile(ProcPath, sprintf('Sub%d', SubID), 'Test\Player', sprintf('PlayerStandStill%d.mat', iRun)),'PlayerStandStill'); 
        ...
        switch  TimePoints
            case 'Movement'
                save(fullfile(ProcPath, sprintf('Sub%d',SubID),'\Test\Player\Backward',sprintf('Backward_Move_Run%d',iRun)),'Backward');
                save(fullfile(ProcPath, sprintf('Sub%d',SubID),'\Test\Player\Backward',sprintf('Forward_Move_Run%d',iRun)),'Forward');
            
            case 'Movement_and_StandStill'
                save(fullfile(ProcPath, sprintf('Sub%d',SubID),'\Test\Player\Backward',sprintf('Backward_Move_and_StandStill_Run%d',iRun)),'Backward');
                save(fullfile(ProcPath, sprintf('Sub%d',SubID),'\Test\Player\Backward',sprintf('Forward_Move_and_StandStill_Run%d',iRun)),'Forward');
            
            case 'StandStill' %aka, StandStill_nonBackward  
                save(fullfile(ProcPath, sprintf('Sub%d',SubID),'\Test\Player\Backward',sprintf('Backward_StandStill_Run%d',iRun)),'Backward');
                save(fullfile(ProcPath, sprintf('Sub%d',SubID),'\Test\Player\Backward',sprintf('Forward_StandStill_Run%d',iRun)),'Forward');
        end
...
[p,stats]           = vartestn([PBackwardYoung, [PBackwardOld;nan(4,1)]], 'TestType','LeveneQuadratic'); 
[h,p,ci,stats]      = ttest2(PBackwardOld, PBackwardYoung)

CohensD             = abs(mean(PBackwardYoung) - mean(PBackwardOld)) /std(PBackwardOld)
...
if ~exist(fullfile(StatsPath,'\Test\Backward\'),'dir')
    mkdir(fullfile(StatsPath,'\Test\Backward\')); end

switch  TimePoints
        case 'Movement'
            save(fullfile(StatsPath,'\Test\Backward\PBackwardYoung_Move'),'PBackwardYoung','nBackwardSubs');
            save(fullfile(StatsPath,'\Test\Backward\PBackwardOld_Move'),'PBackwardOld')
        
        case 'Movement_and_StandStill'
            save(fullfile(StatsPath,'\Test\Backward\PBackwardYoung_Movement_and_StandStill'),'PBackwardYoung','nBackwardSubs');
            save(fullfile(StatsPath,'\Test\Backward\PBackwardOld_Movement_and_StandStill'),'PBackwardOld')
            
        case 'StandStill'
            save(fullfile(StatsPath,'\Test\Backward\PBackwardYoung_StandStill'),'PBackwardYoung','nBackwardSubs');
            save(fullfile(StatsPath,'\Test\Backward\PBackwardOld_StandStill'),'PBackwardOld')
end
...
if ~exist(fullfile(FigPath,'Backward\Test'),'dir')
    mkdir(fullfile(FigPath,'\Backward\Test'))
end


switch  TimePoints
        case 'Movement'
              FigName = fullfile(FigPath,'\Backward\Test','BackwardMovement_Move');
        case 'Movement_and_StandStill'
              FigName = fullfile(FigPath,'\Backward\Test','BackwardMovement_Move_and_StandStill');
        case 'StandStill'
              FigName = fullfile(FigPath,'\Backward\Test','BackwardMovement_StandStill');
end
box off
