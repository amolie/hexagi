function hexagi_behaviour_reg_performance(Subjects,StatsPath,FigPath)
% Regress backward movement, navigational preference and wall bumping on performance. 
...
% Independent variable: backward movements, navigational preference and wall bumping
% grey  = navi preference = x1
% green = backward = x2
% blue  = bumping = x3


%% Load the regressors and performance
...
FigName = fullfile(FigPath,'\Performance','Partial_Regression_Performance_Preference');
fig = gcf;
...
FigName = fullfile(FigPath,'\Performance','Partial_Regression_Performance_Backward');
fig = gcf;
saveas(gcf,FigName,'png')
...
FigName = fullfile(FigPath,'\Performance','Partial_Regression_Performance_WallBumping');
fig = gcf;
saveas(gcf,FigName,'png')
...
plot([CIbYoung(2,1), CIbYoung(2,2)],[1,1],'-','LineWidth',2,'color',[0.4,0.4,0.4]);
plot([CIbYoung(3,1), CIbYoung(3,2)],[2,2],'-','LineWidth',2,'color',[0.0 0.5 0.3]);
plot([CIbYoung(4,1), CIbYoung(4,2)],[3,3],'-','LineWidth',2,'color',[0.3,0.8,0.8]);


%% Old
X = [zscore(NaviStrategy(OldIdx)) zscore(nBackwardSubs(OldIdx)) zscore(nBump(OldIdx)) ]; % Add thigmotaxis 
Y = zscore(Performance(OldIdx));
...
box off
FigName = fullfile(FigPath,'\Performance','Regression_Performance');
fig = gcf;
...
addedvarplot(X,Y,1,InModel) % Holding the other variables constant!
xlabel('Preference | Backward & Wall bumping')
FigName = fullfile(FigPath,'\Performance','Partial_Regression_Preference_Old');
fig = gcf;
saveas(gcf,FigName,'png')
...
addedvarplot(X,Y,2,InModel)
xlabel('Backward | Preference & Wall bumping')
FigName = fullfile(FigPath,'\Performance','Partial_Regression_Backward_Old');
fig = gcf;
saveas(gcf,FigName,'png')
...
addedvarplot(X,Y,3,InModel) 
xlabel('Wall bumping | Backward & Wall bumping')
FigName = fullfile(FigPath,'\Performance','Partial_Regression_WallBumping_Old');
fig = gcf;
saveas(gcf,FigName,'png')
