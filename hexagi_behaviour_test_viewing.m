function hexagi_behaviour_test_viewing(Subjects,Input,Movement,ProcPath,StatsPath,FigPath)
...
fprintf('Evaluating LM viewing and the distribution of viewing directions from the test phase \n')
    ...
    for iRun  = 1:2    
        ...
        switch Movement
            ...
            case 'Move_nonBackward'
                ...
                load(fullfile(ProcPath, sprintf('Sub%d',SubID),'\Test\Player\Backward',sprintf('Forward_Move_and_StandStill_Run%d',iRun)));
                Player = Forward;
            ...
            case 'StandStill_nonBackward'  
                ...
                load(fullfile(ProcPath, sprintf('Sub%d',SubID),'\Test\Player\Backward',sprintf('Forward_StandStill_Run%d',iRun)),'Forward');
                Player = Forward;
        end
        
        
        %% Use the preiviously defined time periode
        
        % Evaluate the entire phase - do nothing
        if Input  == 1
        end
                
        % Evaluate only re-learning / feedback, i.e. from show to grab
        Temp   = [];
        if  Input == 2
            for iTrial          = 1:length(Test.TrialStart) 
                TimeOfInterest  = Player.LocTime >= Test.Show(iTrial) & Player.LocTime <= Test.Grab(iTrial);
                PlayerTestTrial = Player(TimeOfInterest,:);
                Temp            = [Temp ; PlayerTestTrial];
            end
            Player          = Temp;    
        end
        ...
        XDiff                = [double(Test.LMLocX(1)) - Player.LocX ];
        YDiff                = [double(Test.LMLocY(1)) - Player.LocY ];
...
% Save LM viewing - includes outliers!
if ~exist(fullfile(StatsPath,'Test\Viewing'),'dir') 
    mkdir(fullfile(StatsPath,'Test\Viewing')); 
end
save(fullfile(StatsPath,'\Test\Viewing\PLMBoundYaw'),'PLMBoundYaw')
...
%% Barplot showing the probability of LM viewing for the groups
...
% Save the figure 
if ~exist(fullfile(FigPath,'\Viewing\Test'),'dir') 
    mkdir(fullfile(FigPath,'\Viewing\Test')); 
end

switch Movement
    case 'Move_Backward'
        FigName = fullfile(FigPath,'\Viewing\Test','LandmarkViewing_Move_Backward');
    case 'Move_nonBackward'
        FigName = fullfile(FigPath,'\Viewing\Test','LandmarkViewing_Move_nonBackward');
    case 'StandStill_Backward'
        FigName = fullfile(FigPath,'\Viewing\Test','LandmarkViewing_StandStill_Backward');
    case 'StandStill_nonBackward'
        FigName = fullfile(FigPath,'\Viewing\Test','LandmarkViewing_StandStill_nonBackward');
end
box off
